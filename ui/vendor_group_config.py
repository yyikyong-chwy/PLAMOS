...
# local sqlite file (will be synced to S3)
DB_PATH = PROJECT_ROOT / "data" / "planner_groups.db"
DB_PATH.parent.mkdir(parents=True, exist_ok=True)

# S3 settings
S3_BUCKET_NAME = "my-mbm-config-bucket"
S3_DB_KEY = "planner_groups/planner_groups.db"
...
def download_db_from_s3_if_exists():
    """Download DB from S3 if it exists and we don't have a local copy."""
    if DB_PATH.exists():
        return

    s3 = get_s3_client()
    try:
        s3.head_object(Bucket=S3_BUCKET_NAME, Key=S3_DB_KEY)
    except s3.exceptions.ClientError:
        return

    try:
        DB_PATH.parent.mkdir(parents=True, exist_ok=True)
        s3.download_file(S3_BUCKET_NAME, S3_DB_KEY, str(DB_PATH))
    except Exception as e:
        st.warning(f"Could not download DB from S3: {e}")


def upload_db_to_s3():
    """Upload DB to S3 after changes."""
    if not DB_PATH.exists():
        return
    s3 = get_s3_client()
    try:
        s3.upload_file(str(DB_PATH), S3_BUCKET_NAME, S3_DB_KEY)
    except Exception as e:
        st.warning(f"Could not upload DB to S3: {e}")


# ------------ SQLite helpers ------------ #

def get_db_connection() -> sqlite3.Connection:
    download_db_from_s3_if_exists()
...
def init_db_schema(conn: sqlite3.Connection):
    """
    Schema:
      vendor_group_mapping(group_name, vendor_code)
      - group_name: the group (e.g., JaSchack, KNgo, mwilson)
      - vendor_code: vendor assigned to that group
    """
    conn.execute(
        """
...
def save_group_vendors(group_name: str, vendors: List[str]) -> None:
    """Replace all vendors for a group with a new list."""
    conn = get_db_connection()
    try:
        # Delete existing
        conn.execute("DELETE FROM vendor_group_mapping WHERE group_name = ?;", (group_name.strip(),))
        # Insert new
        records = [(group_name.strip(), v.strip()) for v in vendors if v.strip()]
        conn.executemany(
...
def main():
    st.set_page_config(
        page_title="Vendor Group Configuration",
        layout="wide",
    )

    st.title("Vendor Group Configuration")
    ...
    if not all_groups:
        ...
        if selected_group:
            ...
            with col2:
                ...
                if st.button(f"🗑️ Delete Entire Group '{selected_group}'", key="btn_delete_group"):
                    delete_group(selected_group)
                    st.success(f"Deleted group '{selected_group}' and all its vendor assignments.")
                    st.rerun()
    ...
    with col_left:
        new_group_name = st.text_input(
            ...
            placeholder="e.g., 'NewPlanner', 'SpecialGroup'",
        )
    ...
    if st.button("Create Group", type="primary", key="btn_create_group"):
        ...
        if not name:
            ...
            st.error(f"Group '{name}' already exists. Choose a different name.")
        else:
            # Parse vendor codes
    ...
    # ========================================
    # SECTION 3: View All Groups Summary
    # ========================================
    st.markdown("---")
    ...
    st.markdown("---")
    st.caption(
        f"✅ Changes are automatically saved to SQLite (`{DB_PATH.name}`). "
    )
