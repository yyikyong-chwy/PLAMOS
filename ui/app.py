...
# ---------- UI ----------
def main():
    st.set_page_config(page_title="Inventory Data Manager", page_icon="📦", layout="wide")
    st.title("📦 Inventory Data Manager")
    ...
    # ----------------------- DQ: Sub-tabs -----------------------
    with tab_dq:
        st.caption("Data Quality checks and dashboards (coming soon).")
        dq_tab1, dq_tab2 = st.tabs([
            "🔎 DQ — Checks",
            "📈 DQ — Dashboards"
        ])
        with dq_tab1:
            st.info("DQ Checks placeholder — to be implemented.")
        with dq_tab2:
            st.info("DQ Dashboards placeholder — to be implemented.")

    # ----------------------- OUTPUT: Sub-tabs -----------------------
    with tab_output:
        st.caption("Downstream outputs (e.g., PO generation, validations, reports).")
        out_tab1, out_tab2 = st.tabs([
            "🧾 Output — Set A",
            "📑 Output — Set B"
        ])
        with out_tab1:
            st.info("Output Set A placeholder — to be implemented.")
        with out_tab2:
            st.info("Output Set B placeholder — to be implemented.")
